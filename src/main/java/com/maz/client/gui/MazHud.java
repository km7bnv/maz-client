...
public class MazHud {
    ...
    public static void render(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
        ...
        if (enabled(memory)) {Runtime runtime=Runtime.getRuntime();long used=runtime.totalMemory()-runtime.freeMemory(),max=runtime.maxMemory();drawHudBox(graphics,client,"Memory","RAM: "+used/1024/1024+" / "+max/1024/1024+" MB ("+(max>0?(used*100)/max:0)+"%)",pos("Memory",8,30));}
        ...
        Module armor=MazClient.MODULE_MANAGER.getModule("Armor HUD");if(enabled(armor)&&client.player!=null)drawHudBox(graphics,client,"Armor HUD","Armor: "+client.player.getArmorValue(),pos("Armor HUD",8,404));
    ...
    private static void drawHudBox(GuiGraphicsExtractor graphics,Minecraft client,String moduleName,String text,HudLayout.Position p){int alpha=HudLayout.getOpacity(moduleName),width=client.font.width(text)+12;graphics.fill(p.x(),p.y(),p.x()+width,p.y()+18,withAlpha(BACKGROUND,alpha));graphics.fill(p.x(),p.y(),p.x()+3,p.y()+18,withAlpha(ACCENT,alpha));graphics.text(client.font,text,p.x()+7,p.y()+6,adaptiveTextColor(alpha),false);}
