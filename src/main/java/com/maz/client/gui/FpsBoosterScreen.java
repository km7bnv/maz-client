...
public class FpsBoosterScreen extends Screen {
    ...
    private static final int W=560,H=370;
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float d){
        ...
        g.text(font,"FPS Booster Tuning",l+78,t+24,TEXT,false); g.text(font,"Tune how aggressively MazClient reduces rendering work.",l+78,t+42,MUTED,false);
        ...
        int y=t+146; value(g,l+22,r-22,y,"Render Distance",booster==null?"-":booster.getRenderDistance()+" chunks"); value(g,l+22,r-22,y+38,"Simulation Distance",booster==null?"-":booster.getSimulationDistance()+" chunks"); value(g,l+22,r-22,y+76,"Entity Distance",booster==null?"-":booster.getEntityDistancePercent()+"%"); value(g,l+22,r-22,y+114,"Particle Density",booster==null?"-":particleLabel(booster.getParticleKeepEvery())); toggle(g,l+22,r-22,y+152,"Entity Shadows",booster!=null&&booster.getEntityShadows());
    ...
    @Override public boolean mouseClicked(MouseButtonEvent e,boolean dc){if(e.button()!=0||booster==null)return super.mouseClicked(e,dc);int l=(width-W)/2,t=(height-H)/2,r=l+W,b=t+H;double x=e.x(),y=e.y();
        ...
        int row=t+146; if(adjust(x,y,row,r,()->booster.setRenderDistance(booster.getRenderDistance()-1),()->booster.setRenderDistance(booster.getRenderDistance()+1)))return true; if(adjust(x,y,row+38,r,()->booster.setSimulationDistance(booster.getSimulationDistance()-1),()->booster.setSimulationDistance(booster.getSimulationDistance()+1)))return true; if(adjust(x,y,row+76,r,()->booster.setEntityDistancePercent(booster.getEntityDistancePercent()-10),()->booster.setEntityDistancePercent(booster.getEntityDistancePercent()+10)))return true; if(adjust(x,y,row+114,r,()->booster.setParticleKeepEvery(booster.getParticleKeepEvery()+1),()->booster.setParticleKeepEvery(booster.getParticleKeepEvery()-1)))return true;
        if(inside(x,y,r-96,row+157,r-26,row+177)){booster.setEntityShadows(!booster.getEntityShadows());return true;} if(inside(x,y,l+22,b-42,r-22,b-14)){Minecraft.getInstance().gui.setScreen(parent);return true;} return super.mouseClicked(e,dc);
    }
