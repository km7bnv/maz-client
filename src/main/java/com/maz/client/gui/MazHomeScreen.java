...
public class MazHomeScreen extends Screen {
    ...
    @Override public boolean mouseClicked(MouseButtonEvent e,boolean dc){if(e.button()!=0)return super.mouseClicked(e,dc);int l=(width-CARD_WIDTH)/2,t=(height-CARD_HEIGHT)/2,r=l+CARD_WIDTH,b=t+CARD_HEIGHT,bl=l+24,br=r-24,y=t+132,statusLeft=r-164;double x=e.x(),my=e.y();Minecraft c=Minecraft.getInstance();if(inside(x,my,statusLeft,t+25,r-24,t+68)){c.gui.setScreen(new MazMenuScreen());return true;}if(inside(x,my,bl,y,br,y+BUTTON_HEIGHT)){c.gui.setScreen(new SelectWorldScreen(this));return true;}y+=BUTTON_HEIGHT+GAP;if(inside(x,my,bl,y,br,y+BUTTON_HEIGHT)){c.gui.setScreen(new JoinMultiplayerScreen(this));return true;}y+=BUTTON_HEIGHT+GAP;int half=(br-bl-GAP)/2,mid=bl+half;if(inside(x,my,bl,y,mid,y+BUTTON_HEIGHT)){c.gui.setScreen(new OptionsScreen(this,c.options,false));return true;}if(inside(x,my,mid+GAP,y,br,y+BUTTON_HEIGHT)){c.gui.setScreen(new HudEditorScreen());return true;}if(inside(x,my,r-142,b-36,r-88,b-12)){c.gui.setScreen(new MazAboutScreen(this));return true;}if(inside(x,my,r-76,b-36,r-24,b-12)){c.scheduleStop();return true;}return super.mouseClicked(e,dc);}
