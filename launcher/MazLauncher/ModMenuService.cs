...
public sealed class ModMenuService
{
    ...
    private const string MarkerName = ".maz-modmenu";
    private const string ExpectedModId = "modmenu";

    static ModMenuService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        {
            log?.Invoke("Discarding invalid cached Mod Menu JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/modmenu/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible Mod Menu Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("Mod Menu release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("Mod Menu filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("Mod Menu URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed QoL: Mod Menu");
                ...
                try
                {
                    ...
                    if (!IsValidJar(temp)) throw new InvalidDataException("Downloaded Mod Menu file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidJar(target)) throw new InvalidDataException("Mod Menu cache validation failed.");
            ...
            log?.Invoke("Mod Menu Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidJar(cachedPath))
        {
            log?.Invoke("Using validated cached Mod Menu while offline");
        }
    }
