...
public sealed class LanguageReloadService
{
    ...
    private const string MarkerName = ".maz-language-reload";
    private const string ExpectedModId = "language-reload";

    static LanguageReloadService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        {
            log?.Invoke("Discarding invalid cached Language Reload JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/language-reload/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible Language Reload Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("Language Reload release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("Language Reload filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("Language Reload URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed QoL: Language Reload");
                ...
                try
                {
                    ...
                    if (!IsValidJar(temp)) throw new InvalidDataException("Downloaded Language Reload file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidJar(target)) throw new InvalidDataException("Language Reload cache validation failed.");
            ...
            log?.Invoke("Language Reload Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidJar(cachedPath))
        {
            log?.Invoke("Using validated cached Language Reload while offline");
        }
    }
