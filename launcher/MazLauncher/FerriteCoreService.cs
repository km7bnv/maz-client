...
public sealed class FerriteCoreService
{
    ...
    private const string MarkerName = ".maz-ferritecore";
    private const string ExpectedModId = "ferritecore";

    static FerriteCoreService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cachedPath) && File.Exists(cachedPath) && !IsValidFerriteCoreJar(cachedPath))
        {
            log?.Invoke("Discarding invalid cached FerriteCore JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/ferrite-core/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible FerriteCore Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("FerriteCore release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("FerriteCore filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("FerriteCore URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidFerriteCoreJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed optimization: FerriteCore");
                ...
                try
                {
                    ...
                    if (!IsValidFerriteCoreJar(temp)) throw new InvalidDataException("Downloaded FerriteCore file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidFerriteCoreJar(target)) throw new InvalidDataException("FerriteCore cache validation failed.");
            ...
            log?.Invoke("FerriteCore Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidFerriteCoreJar(cachedPath))
        {
            log?.Invoke("Using validated cached FerriteCore while offline");
        }
    }

    private static bool IsValidFerriteCoreJar(string path)
    {
