...
public sealed class ReesesSodiumOptionsService
{
    ...
    private const string MarkerName = ".maz-reeses-sodium-options";
    private const string ExpectedModId = "reeses-sodium-options";

    static ReesesSodiumOptionsService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        {
            log?.Invoke("Discarding invalid cached Reese's Sodium Options JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/reeses-sodium-options/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible Reese's Sodium Options Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("Reese's Sodium Options release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("Reese's Sodium Options filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("Reese's Sodium Options URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed QoL: Reese's Sodium Options");
                ...
                try
                {
                    ...
                    if (!IsValidJar(temp)) throw new InvalidDataException("Downloaded Reese's Sodium Options file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidJar(target)) throw new InvalidDataException("Reese's Sodium Options cache validation failed.");
            ...
            log?.Invoke("Reese's Sodium Options Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidJar(cachedPath))
        {
            log?.Invoke("Using validated cached Reese's Sodium Options while offline");
        }
    }
