...
public sealed class BadOptimizationsService
{
    ...
    private const string MarkerName = ".maz-badoptimizations";
    private const string ExpectedModId = "badoptimizations";

    static BadOptimizationsService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cachedPath) && File.Exists(cachedPath) && !IsValidBadOptimizationsJar(cachedPath))
        {
            log?.Invoke("Discarding invalid cached BadOptimizations JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/badoptimizations/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible BadOptimizations Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            if (stable == null)
                log?.Invoke($"No stable BadOptimizations build is published for Minecraft {LauncherService.MinecraftVersion}; using the newest compatible Fabric beta");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0)
                throw new InvalidDataException("BadOptimizations release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("BadOptimizations filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("BadOptimizations URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidBadOptimizationsJar(target))
            {
                log?.Invoke("Repairing invalid BadOptimizations cache entry");
                TryDelete(target);
            }

            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed optimization: BadOptimizations");
                ...
                try
                {
                    ...
                    if (!IsValidBadOptimizationsJar(temp))
                        throw new InvalidDataException("Downloaded BadOptimizations file is not the expected Fabric mod JAR.");
            ...
            if (!IsValidBadOptimizationsJar(target))
                throw new InvalidDataException("BadOptimizations cache validation failed.");
            ...
            log?.Invoke("BadOptimizations Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName)
                    && File.Exists(cachedPath)
                    && IsValidBadOptimizationsJar(cachedPath))
        {
            log?.Invoke("Using validated cached BadOptimizations while offline");
        }
    }

    private static bool IsValidBadOptimizationsJar(string path)
    {
        ...
        try
        {
            ...
            if (!doc.RootElement.TryGetProperty("id", out var id) || id.ValueKind != JsonValueKind.String) return false;
            return string.Equals(id.GetString(), ExpectedModId, StringComparison.OrdinalIgnoreCase);
        }
        catch
        {
            return false;
