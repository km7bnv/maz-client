...
public sealed class ModernFixService
{
    ...
    private const string MarkerName = ".maz-modernfix";
    private const string ExpectedModId = "modernfix";

    static ModernFixService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cachedPath) && File.Exists(cachedPath) && !IsValidModernFixJar(cachedPath))
        {
            log?.Invoke("Discarding invalid cached ModernFix-mVUS JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/modernfix-mvus/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible ModernFix-mVUS Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("ModernFix-mVUS release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("ModernFix-mVUS filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("ModernFix-mVUS URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidModernFixJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed optimization: ModernFix-mVUS");
                ...
                try
                {
                    ...
                    if (!IsValidModernFixJar(temp)) throw new InvalidDataException("Downloaded ModernFix-mVUS file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidModernFixJar(target)) throw new InvalidDataException("ModernFix-mVUS cache validation failed.");
            ...
            log?.Invoke("ModernFix-mVUS Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidModernFixJar(cachedPath))
        {
            log?.Invoke("Using validated cached ModernFix-mVUS while offline");
        }
    }

    private static bool IsValidModernFixJar(string path)
    {
