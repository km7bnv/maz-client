...
public sealed class KryptonService
{
    ...
    private const string MarkerName = ".maz-krypton";
    private const string ExpectedModId = "krypton";

    static KryptonService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cachedPath) && File.Exists(cachedPath) && !IsValidKryptonJar(cachedPath))
        {
            log?.Invoke("Discarding invalid cached Krypton JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/krypton/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible Krypton Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("Krypton release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("Krypton filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("Krypton URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidKryptonJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed optimization: Krypton");
                ...
                try
                {
                    ...
                    if (!IsValidKryptonJar(temp)) throw new InvalidDataException("Downloaded Krypton file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidKryptonJar(target)) throw new InvalidDataException("Krypton cache validation failed.");
            ...
            log?.Invoke("Krypton Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidKryptonJar(cachedPath))
        {
            log?.Invoke("Using validated cached Krypton while offline");
        }
    }

    private static bool IsValidKryptonJar(string path)
    {
