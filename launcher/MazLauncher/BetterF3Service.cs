...
public sealed class BetterF3Service
{
    ...
    private const string MarkerName = ".maz-betterf3";
    private const string ExpectedModId = "betterf3";

    static BetterF3Service()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        {
            log?.Invoke("Discarding invalid cached BetterF3 JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/betterf3/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible BetterF3 Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("BetterF3 release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("BetterF3 filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("BetterF3 URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed QoL: BetterF3");
                ...
                try
                {
                    ...
                    if (!IsValidJar(temp)) throw new InvalidDataException("Downloaded BetterF3 file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidJar(target)) throw new InvalidDataException("BetterF3 cache validation failed.");
            ...
            log?.Invoke("BetterF3 Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidJar(cachedPath))
        {
            log?.Invoke("Using validated cached BetterF3 while offline");
        }
    }
