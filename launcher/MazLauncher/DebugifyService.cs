...
public sealed class DebugifyService
{
    ...
    private const string MarkerName = ".maz-debugify";
    private const string ExpectedModId = "debugify";

    static DebugifyService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cachedPath) && File.Exists(cachedPath) && !IsValidDebugifyJar(cachedPath))
        {
            log?.Invoke("Discarding invalid cached Debugify JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/debugify/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible Debugify Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("Debugify release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("Debugify filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("Debugify URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidDebugifyJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed bug-fix mod: Debugify");
                ...
                try
                {
                    ...
                    if (!IsValidDebugifyJar(temp)) throw new InvalidDataException("Downloaded Debugify file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidDebugifyJar(target)) throw new InvalidDataException("Debugify cache validation failed.");
            ...
            log?.Invoke("Debugify Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidDebugifyJar(cachedPath))
        {
            log?.Invoke("Using validated cached Debugify while offline");
        }
    }

    private static bool IsValidDebugifyJar(string path)
    {
