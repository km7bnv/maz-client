...
public sealed class ImmediatelyFastService
{
    ...
    private const string MarkerName = ".maz-immediatelyfast";
    private const string ExpectedModId = "immediatelyfast";

    static ImmediatelyFastService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cachedPath) && File.Exists(cachedPath) && !IsValidImmediatelyFastJar(cachedPath))
        {
            log?.Invoke("Discarding invalid cached ImmediatelyFast JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/immediatelyfast/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible ImmediatelyFast Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("ImmediatelyFast release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("ImmediatelyFast filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("ImmediatelyFast URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidImmediatelyFastJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed optimization: ImmediatelyFast");
                ...
                try
                {
                    ...
                    if (!IsValidImmediatelyFastJar(temp)) throw new InvalidDataException("Downloaded ImmediatelyFast file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidImmediatelyFastJar(target)) throw new InvalidDataException("ImmediatelyFast cache validation failed.");
            ...
            log?.Invoke("ImmediatelyFast Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidImmediatelyFastJar(cachedPath))
        {
            log?.Invoke("Using validated cached ImmediatelyFast while offline");
        }
    }

    private static bool IsValidImmediatelyFastJar(string path)
    {
