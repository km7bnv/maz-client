...
public sealed class SodiumReliefService
{
    ...
    private const string MarkerName = ".maz-sodiumrelief";
    private const string ExpectedModId = "sodiumrelief";

    static SodiumReliefService()
    {
    ...
    public async Task EnsureForMazClientAsync(string mazClientVersion, Action<string>? log = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cachedPath) && File.Exists(cachedPath) && !IsValidSodiumReliefJar(cachedPath))
        {
            log?.Invoke("Discarding invalid cached Sodium Relief JAR");
        ...
        try
        {
            var query = $"https://api.modrinth.com/v2/project/sodium-relief/version?loaders=%5B%22fabric%22%5D&game_versions=%5B%22{LauncherService.MinecraftVersion}%22%5D";
            ...
            if (selected == null)
                throw new InvalidOperationException($"No compatible Sodium Relief Fabric release or beta is available for Minecraft {LauncherService.MinecraftVersion}.");

            var files = selected.Value.GetProperty("files");
            if (files.GetArrayLength() == 0) throw new InvalidDataException("Sodium Relief release contains no files.");
            ...
            var fileName = selectedFile.GetProperty("filename").GetString() ?? throw new InvalidDataException("Sodium Relief filename missing.");
            var url = selectedFile.GetProperty("url").GetString() ?? throw new InvalidDataException("Sodium Relief URL missing.");
            var target = Path.Combine(modsDir, fileName);

            if (File.Exists(target) && !IsValidSodiumReliefJar(target)) TryDelete(target);
            if (!File.Exists(target))
            {
                log?.Invoke("Downloading managed optimization: Sodium Relief");
                ...
                try
                {
                    ...
                    if (!IsValidSodiumReliefJar(temp)) throw new InvalidDataException("Downloaded Sodium Relief file is not the expected Fabric mod JAR.");
                    File.Move(temp, target, true);
                }
                finally { TryDelete(temp); }
            }

            if (!IsValidSodiumReliefJar(target)) throw new InvalidDataException("Sodium Relief cache validation failed.");
            ...
            log?.Invoke("Sodium Relief Fabric JAR verified and cached");
        }
        catch when (!string.IsNullOrWhiteSpace(cachedName) && File.Exists(cachedPath) && IsValidSodiumReliefJar(cachedPath))
        {
            log?.Invoke("Using validated cached Sodium Relief while offline");
        }
    }

    private static bool IsValidSodiumReliefJar(string path)
    {
